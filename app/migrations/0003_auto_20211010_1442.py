# Generated by Django 3.2.8 on 2021-10-10 17:42

from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('app', '0002_alter_competidor_options'),
    ]

    operations = [
        migrations.AlterField(
            model_name='competidor',
            name='academia',
            field=models.CharField(default=True, max_length=100),
        ),
        migrations.AlterField(
            model_name='competidor',
            name='apellido',
            field=models.CharField(default=True, max_length=30),
        ),
        migrations.AlterField(
            model_name='competidor',
            name='categoria',
            field=models.CharField(choices=[('Kids (Hasta 7 años)', 'Kids (Hasta 7 años)'), ('Infantil A (8-9 años)', 'Infantil A (8-9 años)'), ('Infantil B (10-11 años)', 'Infantil B (10-11 años)'), ('Cadete (12-13 años)', 'Cadete (12-13 años)'), ('Juvenil A (14-15 años)', 'Juvenil A (14-15 años)'), ('Juvenil B (16-17 años)', 'Juvenil B (16-17 años)'), ('Adultos (18-35 años)', 'Adultos (18-35 años)'), ('Seniors (+35 años)', 'Seniors (+35 años)')], default=True, max_length=100),
        ),
        migrations.AlterField(
            model_name='competidor',
            name='celular',
            field=models.IntegerField(default=True),
        ),
        migrations.AlterField(
            model_name='competidor',
            name='dni',
            field=models.IntegerField(default=True, unique=True),
        ),
        migrations.AlterField(
            model_name='competidor',
            name='graduacion',
            field=models.CharField(choices=[('Blanco - Blanco Punta Amarilla', 'Blanco - Blanco Punta Amarilla'), ('Amarillo - Verde Punta Azul', 'Amarillo - Verde Punta Azul'), ('Azul - Rojo Punta Negra', 'Azul - Rojo Punta Negra'), ('I Dan', 'I Dan'), ('II Dan', 'II Dan'), ('III Dan', 'III Dan'), ('IV Dan - VI Dan', 'IV Dan - VI Dan')], default=True, max_length=100),
        ),
        migrations.AlterField(
            model_name='competidor',
            name='nombre',
            field=models.CharField(default=True, max_length=30),
        ),
    ]
